              ...
              {Array.from({ length: count }, (_, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <input value={names[i]} onChange={e => setNames(ns => ns.map((x, k) => k === i ? e.target.value : x))}
                    placeholder={`Joueur ${i + 1}`} style={{ flex: 1, background: C.card2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "11px 13px", color: C.text, fontSize: 15, boxSizing: "border-box" }} />
                </div>
              ))}
            </div>
          </div>
