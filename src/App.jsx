...
const TYPES_FLECHETTES = [
  { v:"electronique", l:"⚡ Électronique" },
  { v:"traditionnel", l:"🎯 Traditionnel" },
  { v:"trad-auto", l:"🎯📊 Traditionnel auto-scoring" },
];
const typeLabel = (t) => TYPES_FLECHETTES.find(x=>x.v===t)?.l || t;
const typeBadgeColor = (t) => t==="electronique" ? C.accent : t==="trad-auto" ? "#34d399" : "#60a5fa";
...
const db = {
  ...
  addAssociation: (data) => sb("associations", { method:"POST", body: JSON.stringify(data) }),
  updateAssociation: (slug, data) => sb(`associations?slug=eq.${encodeURIComponent(slug)}`, { method:"PATCH", body: JSON.stringify(data), prefer:"return=minimal" }),
  ...
  addTournoi: (data) => sb("tournois", { method:"POST", body: JSON.stringify(data) }),
  updateTournoi: (slug, data) => sb(`tournois?slug=eq.${encodeURIComponent(slug)}`, { method:"PATCH", body: JSON.stringify(data), prefer:"return=minimal" }),
...
function LeafletMap({ bars=[], associations=[], tournois=[], onBarClick, onAssoClick, onTournoiClick, centerSlug, centerVille=null, height=400 }) {
  ...
  useEffect(() => {
    ...
    bars.forEach(bar => {
      ...
      m.bindPopup(`<div style="font-family:Inter,sans-serif;min-width:160px"><strong>${bar.nom}</strong><br><span style="color:#64748b;font-size:12px">📍 ${bar.ville}</span><br><button style="margin-top:8px;background:#f97316;color:#fff;border:none;border-radius:6px;padding:5px 12px;cursor:pointer;font-size:12px;font-weight:600" onclick="window.__dpBar('${bar.slug}')">Voir la fiche →</button></div>`);
    ...
    associations.forEach(asso => {
      ...
      const icon = L.divIcon({ className:"", html:`<div style="width:${isHL?38:28}px;height:${isHL?38:28}px;background:${isHL?"#fff":"#7c3aed"};border:3px solid ${isHL?"#7c3aed":"rgba(255,255,255,0.3)"};border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:${isHL?20:14}px;box-shadow:0 2px 8px rgba(0,0,0,0.5);cursor:pointer">🫂</div>`, iconSize:[isHL?38:28,isHL?38:28], iconAnchor:[isHL?19:14,isHL?19:14] });
      const m = L.marker([asso.lat,asso.lng],{icon}).addTo(map);
      m.bindPopup(`<div style="font-family:Inter,sans-serif;min-width:160px"><strong>${asso.nom}</strong><br><span style="color:#64748b;font-size:12px">📍 ${asso.ville}</span><br><span style="font-size:12px">🗓 ${asso.jours}</span>${onAssoClick?`<br><button style="margin-top:8px;background:#7c3aed;color:#fff;border:none;border-radius:6px;padding:5px 12px;cursor:pointer;font-size:12px;font-weight:600" onclick="window.__dpAsso('${asso.slug}')">Voir la fiche →</button>`:""}</div>`);
      markersRef.current.push(m);
    });
    ...
    tournois.forEach(t => {
      ...
      const icon = L.divIcon({ className:"", html:`<div style="width:28px;height:28px;background:${isPast?"#444":C.yellow};border:3px solid rgba(255,255,255,0.3);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;box-shadow:0 2px 8px rgba(0,0,0,0.5);cursor:pointer">🏅</div>`, iconSize:[28,28], iconAnchor:[14,14] });
      const m = L.marker([t.lat,t.lng],{icon}).addTo(map);
      m.bindPopup(`<div style="font-family:Inter,sans-serif;min-width:160px"><strong>${t.nom}</strong><br><span style="color:#64748b;font-size:12px">📍 ${t.ville}</span><br><span style="font-size:12px">📅 ${new Date(t.date).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"})}</span>${onTournoiClick?`<br><button style="margin-top:8px;background:#f59e0b;color:#fff;border:none;border-radius:6px;padding:5px 12px;cursor:pointer;font-size:12px;font-weight:600" onclick="window.__dpTournoi('${t.slug}')">Voir →</button>`:""}</div>`);
      markersRef.current.push(m);
    });

    if (centerSlug) {
      const b=[...bars,...associations,...tournois].find(x=>x.slug===centerSlug);
      if(b?.lat) map.flyTo([b.lat,b.lng],15,{duration:0.8});
    }
    window.__dpBar = slug => onBarClick && onBarClick(slug);
    window.__dpAsso = slug => onAssoClick && onAssoClick(slug);
    window.__dpTournoi = slug => onTournoiClick && onTournoiClick(slug);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const all = [...bars,...associations,...tournois].filter(b => b.lat && b.lng && b.ville?.toLowerCase().includes(q));
      if (all.length === 1) { map.flyTo([all[0].lat, all[0].lng], 14, {duration:0.8}); return; }
      if (all.length > 1) { const bounds = L.latLngBounds(all.map(b=>[b.lat,b.lng])); map.flyToBounds(bounds, {padding:[60,60], duration:0.8, maxZoom:14}); return; }
...
const Btn = ({ children, onClick, variant="primary", style={}, disabled=false }) => {
  ...
  const v = { primary:{background:C.accent,color:"#fff"}, ghost:{background:"transparent",color:C.accent,border:`1px solid ${C.accent}`}, dark:{background:C.card,color:C.text,border:`1px solid ${C.border}`}, danger:{background:"#7f1d1d",color:C.red,border:`1px solid ${C.red}44`}, success:{background:"#14532d",color:C.green,border:`1px solid ${C.green}44`}, purple:{background:"#4c1d95",color:C.purple,border:`1px solid ${C.purple}44`} };
...
// ── LÉGENDE CARTE ─────────────────────────────────────────────────────────────
const MapLegend = ({ showBars=true, showAssos=false, showTournois=false }) => (
  <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginTop:8, marginBottom:4 }}>
    {showBars && <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:C.muted }}><span style={{ background:C.accent, borderRadius:"50%", width:18, height:18, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:10 }}>🎯</span> Bars</div>}
    {showAssos && <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:C.muted }}><span style={{ background:"#7c3aed", borderRadius:"50%", width:18, height:18, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:10 }}>🫂</span> Associations</div>}
    {showTournois && <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:12, color:C.muted }}><span style={{ background:C.yellow, borderRadius:"50%", width:18, height:18, display:"inline-flex", alignItems:"center", justifyContent:"center", fontSize:10 }}>🏅</span> Tournois</div>}
  </div>
);
...
const Nav = ({ page, setPage, isAdmin }) => {
  ...
  const links = [["bars","🎯 Bars"],["associations","🫂 Assoc."],["tournois","🏅 Tournois"],["proposer","➕ Bar"],["proposer-asso","🤝 Asso"],["proposer-tournoi","🏅 Tournoi"],["apropos","ℹ️ À propos"],["contact","✉️ Contact"]];
  return (
...
const BarCard = ({ bar, onClick }) => (
  <div onClick={onClick} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:18, cursor:"pointer", transition:"border-color .15s", display:"flex", flexDirection:"column", gap:9 }}
    ...
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:6 }}>
      ...
      <Badge color={typeBadgeColor(bar.type)}>{typeLabel(bar.type)}</Badge>
    </div>
      ...
      await new Promise(res => {
        ...
        reader.onload = async (ev) => {
          ...
          img.onload = async () => {
            ...
            try { const added = await db.addPhoto({ bar_slug:barSlug, pseudo:pseudo.trim()||"Anonyme", data:canvas.toDataURL("image/jpeg",0.7), date:Date.now() }); if(added?.[0]) setPhotos(p=>[added[0],...p]); } catch {}
            res();
    ...
    <div style={{ marginBottom:24 }}>
      ...
      {lightbox!==null && (
        <div onClick={()=>setLightbox(null)} style={{ position:"fixed", inset:0, background:"#000d", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", cursor:"zoom-out" }}>
          <div style={{ position:"relative", maxWidth:"90vw" }}>
            ...
            {photos.length>1&&<><button onClick={e=>{e.stopPropagation();setLightbox((lightbox-1+photos.length)%photos.length);}} style={{ position:"absolute",left:-44,top:"50%",transform:"translateY(-50%)",background:"#fff2",border:"none",color:"#fff",fontSize:24,cursor:"pointer",borderRadius:6,padding:"4px 10px" }}>‹</button><button onClick={e=>{e.stopPropagation();setLightbox((lightbox+1)%photos.length);}} style={{ position:"absolute",right:-44,top:"50%",transform:"translateY(-50%)",background:"#fff2",border:"none",color:"#fff",fontSize:24,cursor:"pointer",borderRadius:6,padding:"4px 10px" }}>›</button></>}
          </div>
        </div>
      )}
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:18, marginBottom:14 }}>
        <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
          ...
          <Btn onClick={()=>fileRef.current?.click()} variant="ghost" style={{ fontSize:13, padding:"8px 16px" }} disabled={uploading}>{uploading?"⏳ Envoi…":"📷 Ajouter"}</Btn>
        </div>
      </div>
      {loading?<Spinner/>:photos.length===0?<div style={{ textAlign:"center", padding:"24px", background:C.card, border:`1px dashed ${C.border}`, borderRadius:12 }}><p style={{ color:C.muted, fontSize:13 }}>📷 Aucune photo pour l'instant.</p></div>:(
        <div style={{ columns:"repeat(auto-fill, minmax(140px, 1fr))", gap:8 }}>
          {photos.map((p,i)=>(
            <div key={p.id} style={{ breakInside:"avoid", marginBottom:8, position:"relative", borderRadius:10, overflow:"hidden", cursor:"zoom-in", border:`1px solid ${C.border}` }} onClick={()=>setLightbox(i)}>
              <img src={p.data} alt="" style={{ width:"100%", display:"block" }}/>
              <div style={{ position:"absolute", bottom:0, left:0, right:0, background:"linear-gradient(transparent,#000a)", padding:"12px 6px 4px", fontSize:10, color:"#ccc" }}>📷 {p.pseudo}</div>
              {isAdmin&&<button onClick={e=>{e.stopPropagation();db.deletePhoto(p.id);setPhotos(x=>x.filter(y=>y.id!==p.id));}} style={{ position:"absolute",top:4,right:4,background:"#000a",border:"none",color:C.red,cursor:"pointer",borderRadius:4,padding:"2px 6px",fontSize:11 }}>🗑</button>}
            </div>
          ))}
        </div>
      )}
...
const AvisSection = ({ barSlug, isAdmin }) => {
  ...
  const [loading, setLoading] = useState(true);
  useEffect(() => { Promise.all([db.getAvis(barSlug), db.getReactions(barSlug)]).then(([a,r])=>{ setAvis(a||[]); setReactions(r?.counts||{}); setLoading(false); }).catch(()=>setLoading(false)); }, [barSlug]);
  ...
  const submit = async () => {
    ...
    const added = await db.addAvis({ bar_slug:barSlug, pseudo:form.pseudo.trim()||"Anonyme", texte:form.texte.trim(), reactions:form.reactions, date:Date.now() });
    if(added?.[0]) setAvis(a=>[added[0],...a]);
    const upR = {...reactions}; form.reactions.forEach(r=>{upR[r]=(upR[r]||0)+1;});
  ...
  return (
    <div style={{ marginBottom:20 }}>
      ...
      {Object.values(reactions).some(v=>v>0) && <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:14 }}>{REACTIONS_LIST.filter(r=>reactions[r.id]>0).map(r=><div key={r.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:20, padding:"5px 12px", display:"flex", alignItems:"center", gap:6, fontSize:13 }}><span>{r.emoji}</span><span style={{ color:C.muted }}>{r.label}</span><span style={{ background:C.accent+"33",color:C.accent,borderRadius:10,padding:"1px 7px",fontSize:11,fontWeight:700 }}>{reactions[r.id]}</span></div>)}</div>}
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:18, marginBottom:14 }}>
        <p style={{ fontSize:13, fontWeight:600, marginBottom:10, color:C.muted }}>Laisser un avis</p>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:10 }}>{REACTIONS_LIST.map(r=><button key={r.id} onClick={()=>toggleFormReact(r.id)} style={{ background:form.reactions.includes(r.id)?C.accent+"33":"#111", border:`1px solid ${form.reactions.includes(r.id)?C.accent:C.border}`, borderRadius:20, padding:"5px 11px", cursor:"pointer", fontSize:12, color:form.reactions.includes(r.id)?C.accent:C.muted, display:"flex", alignItems:"center", gap:5 }}>{r.emoji} {r.label}</button>)}</div>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <input value={form.pseudo} onChange={e=>setForm(f=>({...f,pseudo:e.target.value}))} placeholder="Pseudo" style={{ flex:1, minWidth:130, background:"#111", border:`1px solid ${C.border}`, borderRadius:8, padding:"8px 12px", color:C.text, fontSize:13 }}/>
...
const SignalForm = ({ barSlug, barNom, onClose }) => {
  ...
  if(sent) return <div style={{ position:"fixed",inset:0,background:"#000a",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center" }}><div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:32,maxWidth:420,width:"90%",textAlign:"center" }}><div style={{ fontSize:44,marginBottom:10 }}>✅</div><h3 style={{ fontWeight:700,marginBottom:8 }}>Signalement envoyé !</h3><Btn onClick={onClose}>Fermer</Btn></div></div>;
  return (
    <div style={{ position:"fixed",inset:0,background:"#000a",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:28,maxWidth:440,width:"90%" }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
          <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>{[["horaires","⏰"],["ferme","🚫"],["adresse","📍"],["cibles","🎯"],["autre","💬"]].map(([v,l])=><button key={v} onClick={()=>setType(v)} style={{ background:type===v?C.accent+"33":"#111",border:`1px solid ${type===v?C.accent:C.border}`,borderRadius:20,padding:"5px 12px",cursor:"pointer",fontSize:12,color:type===v?C.accent:C.muted }}>{l} {v}</button>)}</div>
...
const ShareBar = ({ bar }) => {
  ...
  return (
    <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:20 }}>
      ...
      <Btn onClick={()=>{try{navigator.clipboard.writeText(url);}catch{}setCopied(true);setTimeout(()=>setCopied(false),2000);}} variant="dark" style={{ fontSize:12, padding:"7px 14px" }}>{copied?"✅ Copié !":"🔗 Copier"}</Btn>
    </div>
  );
};
...
const EditBarModal = ({ bar, onSave, onClose }) => {
  ...
  const save=async()=>{ setSaving(true); const data={ nom:f.nom,ville:f.ville,cp:f.cp,adresse:f.adresse,tel:f.tel,type:f.type,cibles:parseInt(f.cibles)||1,horaires:f.horaires,description:f.description,tournois:f.tournois==="oui",lat:parseFloat(f.lat)||null,lng:parseFloat(f.lng)||null }; await db.updateBar(bar.slug,data); onSave({...bar,...data}); setSaving(false); onClose(); };
  return (
...
const EditAssoModal = ({ asso, onSave, onClose }) => {
  ...
  const [saving,setSaving]=useState(false);
  const set=k=>v=>setF(p=>({...p,[k]:v}));
  const save=async()=>{ setSaving(true); const data={ nom:f.nom,ville:f.ville,zone:f.zone,type:f.type,jours:f.jours,lieu:f.lieu,tel:f.tel,contact:f.contact,description:f.description,lat:parseFloat(f.lat)||null,lng:parseFloat(f.lng)||null }; await db.updateAssociation(asso.slug,data); onSave({...asso,...data}); setSaving(false); onClose(); };
  return (
    <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:24,maxWidth:600,width:"100%",maxHeight:"90vh",overflowY:"auto" }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Zone" value={f.zone} onChange={set("zone")} placeholder="Côte Basque"/><Field label="Type" as="select" value={f.type} onChange={set("type")} options={[{v:"electronique",l:"⚡ Électronique"},{v:"traditionnel",l:"🎯 Traditionnel"}]} placeholder=""/></div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Jours d'entraînement" value={f.jours} onChange={set("jours")} placeholder="Vendredi 20h"/><Field label="Lieu" value={f.lieu} onChange={set("lieu")} placeholder="Bar des Sports"/></div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Téléphone" value={f.tel} onChange={set("tel")} placeholder="06 XX XX XX XX"/><Field label="Contact" value={f.contact} onChange={set("contact")} placeholder="email ou site"/></div>
          <Field label="Description" value={f.description} onChange={set("description")} placeholder="Description…" as="textarea"/>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Latitude GPS" value={f.lat} onChange={set("lat")} placeholder="43.49" type="number"/><Field label="Longitude GPS" value={f.lng} onChange={set("lng")} placeholder="-1.47" type="number"/></div>
          <div style={{ display:"flex",gap:10,marginTop:8 }}><Btn onClick={save} disabled={saving||!f.nom||!f.ville} style={{ flex:1 }}>{saving?"Sauvegarde…":"💾 Sauvegarder"}</Btn><Btn onClick={onClose} variant="dark" style={{ flex:1 }}>Annuler</Btn></div>
        </div>
    ...
    <div style={{ position:"fixed",inset:0,background:"#000c",zIndex:600,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:24,maxWidth:620,width:"100%",maxHeight:"90vh",overflowY:"auto" }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Date" value={f.date} onChange={set("date")} placeholder="2025-09-20" type="date"/><Field label="Bar organisateur" value={f.bar} onChange={set("bar")} placeholder="Le Central"/></div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12 }}>
            <Field label="Type" as="select" value={f.type} onChange={set("type")} options={[{v:"electronique",l:"⚡ Électronique"},{v:"traditionnel",l:"🎯 Traditionnel"}]} placeholder=""/>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Prix d'inscription" value={f.prix} onChange={set("prix")} placeholder="5€"/><Field label="Nombre de places" value={f.places} onChange={set("places")} placeholder="32"/></div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Contact inscription" value={f.contact} onChange={set("contact")} placeholder="email ou tél"/><Field label="Lien (site, Facebook…)" value={f.lien} onChange={set("lien")} placeholder="https://..."/></div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}><Field label="Latitude GPS" value={f.lat} onChange={set("lat")} placeholder="43.49" type="number"/><Field label="Longitude GPS" value={f.lng} onChange={set("lng")} placeholder="-1.47" type="number"/></div>
          <div style={{ display:"flex",gap:10,marginTop:8 }}><Btn onClick={save} disabled={saving||!f.nom||!f.ville} style={{ flex:1 }}>{saving?"Sauvegarde…":"💾 Sauvegarder"}</Btn><Btn onClick={onClose} variant="dark" style={{ flex:1 }}>Annuler</Btn></div>
        </div>
      </div>
  ...
  return (
    <div>
      <div style={{ background:"linear-gradient(135deg,#111 0%,#1a0800 100%)", padding:"64px 20px 48px", textAlign:"center" }}>
        <div style={{ maxWidth:680, margin:"0 auto" }}>
          ...
          <div style={{ position:"relative", maxWidth:460, margin:"0 auto" }}>
            ...
            {results.length>0 && <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, right:0, background:"#1a1a1a", border:`1px solid ${C.border}`, borderRadius:10, zIndex:50, overflow:"hidden" }}>{results.map(b=><div key={b.id} onClick={()=>{setBarSlug(b.slug);setPage("bar");}} style={{ padding:"11px 16px", cursor:"pointer", borderBottom:`1px solid ${C.border}`, display:"flex", justifyContent:"space-between" }} onMouseEnter={e=>e.currentTarget.style.background="#222"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><span>{b.nom} {b.verifie&&"✅"}</span><span style={{ color:C.muted, fontSize:12 }}>📍 {b.ville}</span></div>)}</div>}
          </div>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"40px 20px 0" }}>
        <h2 style={{ fontWeight:700, fontSize:20, marginBottom:8 }}>🗺️ Carte des bars</h2>
        <MapLegend showBars showAssos showTournois/>
        <LeafletMap bars={mapBars} associations={associations} tournois={tournois} onBarClick={(slug)=>{setBarSlug(slug);setPage("bar");}} onAssoClick={(slug)=>{setAssoSlug(slug);setPage("asso");}} onTournoiClick={(slug)=>{setTournoiSlug(slug);setPage("tournoi-detail");}} centerSlug={null} centerVille={search.trim()||null} height={400}/>
      </div>
      ...
      <div style={{ background:"#111", padding:"32px 20px" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {villes.map(v=>{ const nb=bars.filter(b=>b.ville===v).length; return <button key={v} onClick={()=>{setVilleFilter(v);setPage("bars");}} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:20, padding:"7px 15px", color:C.text, cursor:"pointer", fontSize:13, fontWeight:500, display:"flex", alignItems:"center", gap:6 }} onMouseEnter={e=>e.currentTarget.style.borderColor=C.accent} onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>{v} <span style={{ background:C.accent+"33",color:C.accent,borderRadius:10,padding:"0px 7px",fontSize:11,fontWeight:700 }}>{nb}</span></button>; })}
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"36px 20px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:14 }}>
          {[["📍","Proposer un bar","Vous connaissez un bar ?","proposer",C.accent],["🫂","Proposer une association","Vous connaissez un club ?","proposer-asso",C.purple],["🏅","Proposer un tournoi","Organisez-vous un tournoi ?","proposer-tournoi",C.yellow]].map(([e,t,d,p,c])=>(
            <div key={p} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"24px 18px", textAlign:"center" }}>
              ...
              <Btn onClick={()=>setPage(p)} style={{ fontSize:13, background:c }}>Proposer</Btn>
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap" }}>
        ...
        <select value={type} onChange={e=>setType(e.target.value)} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, padding:"9px 12px", color:C.text, fontSize:14 }}>
          ...
          {TYPES_FLECHETTES.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}
        </select>
...
const BarDetail = ({ slug, allBars, associations, setBars, setPage, setAssoSlug, isAdmin }) => {
  const [bar,setBar]=useState(null); const [loading,setLoading]=useState(true); const [showSignal,setShowSignal]=useState(false); const [showEdit,setShowEdit]=useState(false);
  useEffect(()=>{ db.getBar(slug).then(async b=>{ if(b){ setBar(b); const nv=(b.vues||0)+1; db.updateBarVues(slug,b.vues||0); setBar({...b,vues:nv}); setBars(p=>p.map(x=>x.slug===slug?{...x,vues:nv}:x)); } setLoading(false); }).catch(()=>setLoading(false)); },[slug]);
  ...
  return (
    <div style={{ maxWidth:860, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:10,marginBottom:6 }}>
        <div style={{ display:"flex",alignItems:"center",gap:10,flexWrap:"wrap" }}><h1 style={{ fontWeight:800,fontSize:28 }}>{bar.nom}</h1>{bar.verifie&&<Badge color={C.green}>✅ Vérifié</Badge>}</div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:12,marginBottom:16 }}>
        <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18 }}>
          ...
          {[["📍","Adresse",(bar.adresse||"—")+", "+bar.ville],["⏰","Horaires",bar.horaires||"Non renseignés"],["📞","Téléphone",bar.tel||"Non renseigné"]].map(([i,l,v])=><div key={l} style={{ display:"flex",gap:8,marginBottom:10 }}><span style={{ fontSize:15 }}>{i}</span><div><div style={{ fontSize:11,color:C.muted,marginBottom:1 }}>{l}</div><div style={{ fontSize:13 }}>{v}</div></div></div>)}
        </div>
        <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18 }}>
          <h3 style={{ fontWeight:700,marginBottom:12,color:C.accent,fontSize:14 }}>🎯 Équipement</h3>
          {[["Type",bar.type==="electronique"?"Électronique":"Traditionnel"],["Cibles",bar.cibles+" cible"+(bar.cibles>1?"s":"")],["Tournois",bar.tournois?"✅ Oui":"❌ Non"]].map(([l,v])=><div key={l} style={{ display:"flex",justifyContent:"space-between",borderBottom:`1px solid ${C.border}`,paddingBottom:7,marginBottom:7 }}><span style={{ color:C.muted,fontSize:12 }}>{l}</span><span style={{ fontWeight:500,fontSize:13 }}>{v}</span></div>)}
        </div>
      </div>
      ...
      {bar.lat&&<div style={{ marginBottom:16 }}><LeafletMap bars={allBars} onBarClick={()=>{}} centerSlug={bar.slug} height={200}/></div>}
      <ShareBar bar={bar}/>
      <div style={{ display:"flex",gap:10,flexWrap:"wrap",marginBottom:28 }}>
        <a href={`https://www.google.com/maps/search/${encodeURIComponent((bar.adresse||bar.nom)+" "+bar.ville)}`} target="_blank" rel="noreferrer"><Btn variant="ghost" style={{ fontSize:12 }}>🗺️ Maps</Btn></a>
        <Btn variant="dark" onClick={()=>setShowSignal(true)} style={{ fontSize:12 }}>⚠️ Signaler</Btn>
  ...
  return (
    <div style={{ maxWidth:1100, margin:"0 auto", padding:"36px 20px" }}>
      ...
      {view==="carte"?(
        <div style={{ marginBottom:20 }}>
          <MapLegend showAssos/>
          <LeafletMap associations={associations} onAssoClick={(slug)=>{setAssoSlug(slug);setPage("asso");}} centerSlug={null} height={450}/>
        </div>
      ):(
    ...
    <div style={{ maxWidth:860, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:12,marginBottom:16 }}>
        <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18 }}>
          ...
          {[["🗓","Entraînements",asso.jours],["📍","Lieu",asso.lieu],["📞","Téléphone",asso.tel||"Non renseigné"],["📧","Contact",asso.contact]].map(([i,l,v])=><div key={l} style={{ display:"flex",gap:8,marginBottom:10 }}><span>{i}</span><div><div style={{ fontSize:11,color:C.muted,marginBottom:1 }}>{l}</div><div style={{ fontSize:13 }}>{v}</div></div></div>)}
        </div>
        <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18 }}>
      ...
      {asso.lat&&<LeafletMap associations={[asso]} centerSlug={asso.slug} height={180}/>}
    </div>
  );
};
...
const Tournois = ({ tournois, setPage, setTournoiSlug }) => {
  ...
  const TCard=({t})=>{
    ...
    return (
      <div onClick={()=>{setTournoiSlug(t.slug);setPage("tournoi-detail");}}
        style={{ background:C.card,border:`1px solid ${isPast?C.border:C.yellow+"44"}`,borderRadius:12,padding:20,opacity:isPast?.7:1,cursor:"pointer",transition:"border-color .15s" }}
        ...
        <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
          <Badge color={t.type==="electronique"?C.accent:"#60a5fa"}>{t.type==="electronique"?"⚡":"🎯"}</Badge>
  ...
  return (
    <div style={{ maxWidth:1000, margin:"0 auto", padding:"36px 20px" }}>
      ...
      {view==="carte"?(
        <div style={{ marginBottom:20 }}>
          <MapLegend showTournois/>
          <LeafletMap tournois={tournois} onTournoiClick={(slug)=>{setTournoiSlug(slug);setPage("tournoi-detail");}} centerSlug={null} height={450}/>
        </div>
      ):(
        <>
  ...
  return (
    <div style={{ maxWidth:860, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:12,marginBottom:16 }}>
        <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18 }}>
          <h3 style={{ fontWeight:700,marginBottom:12,color:C.accent,fontSize:14 }}>📋 Infos pratiques</h3>
          {[["📍","Ville",t.ville],["🍺","Bar",t.bar||"—"],["🏆","Association",t.association||"—"],["📞","Contact",t.contact||"—"]].map(([i,l,v])=><div key={l} style={{ display:"flex",gap:8,marginBottom:10 }}><span>{i}</span><div><div style={{ fontSize:11,color:C.muted,marginBottom:1 }}>{l}</div><div style={{ fontSize:13 }}>{v}</div></div></div>)}
        </div>
        <div style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18 }}>
          <h3 style={{ fontWeight:700,marginBottom:12,color:C.accent,fontSize:14 }}>🎯 Format</h3>
          {[["Type",t.type==="electronique"?"⚡ Électronique":"🎯 Traditionnel"],["Format",t.format||"—"],["Niveau",t.niveau==="tous"?"Tous niveaux":t.niveau||"—"],["Prix",t.prix||"Gratuit"],["Places",t.places||"Non limité"]].map(([l,v])=><div key={l} style={{ display:"flex",justifyContent:"space-between",borderBottom:`1px solid ${C.border}`,paddingBottom:7,marginBottom:7 }}><span style={{ color:C.muted,fontSize:12 }}>{l}</span><span style={{ fontWeight:500,fontSize:13 }}>{v}</span></div>)}
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth:660, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:13 }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:13 }}><Field label="Code postal" value={f.cp} onChange={set("cp")} placeholder="64100"/><Field label="Type" as="select" value={f.type} onChange={set("type")} options={[{v:"electronique",l:"⚡ Électronique"},{v:"traditionnel",l:"🎯 Traditionnel"},{v:"les deux",l:"Les deux"}]} placeholder=""/></div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:13 }}><Field label="Cibles" value={f.cibles} onChange={set("cibles")} placeholder="2" type="number"/><Field label="Tournois ?" as="select" value={f.tournois} onChange={set("tournois")} options={[{v:"non",l:"Non"},{v:"oui",l:"Oui"},{v:"nsp",l:"Je ne sais pas"}]} placeholder=""/></div>
  ...
  return (
    <div style={{ maxWidth:660, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:13 }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:13 }}><Field label="Zone / département" value={f.zone} onChange={set("zone")} placeholder="Côte Basque"/><Field label="Type" as="select" value={f.type} onChange={set("type")} options={[{v:"electronique",l:"⚡ Électronique"},{v:"traditionnel",l:"🎯 Traditionnel"}]} placeholder=""/></div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:13 }}><Field label="Jours d'entraînement" value={f.jours} onChange={set("jours")} placeholder="Vendredi 20h"/><Field label="Lieu" value={f.lieu} onChange={set("lieu")} placeholder="Bar des Sports"/></div>
  ...
  return (
    <div style={{ maxWidth:660, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:13 }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:13 }}><Field label="Date *" value={f.date} onChange={set("date")} placeholder="" type="date"/><Field label="Bar organisateur" value={f.bar} onChange={set("bar")} placeholder="Le Central"/></div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:13 }}><Field label="Prix d'inscription" value={f.prix} onChange={set("prix")} placeholder="5€ / Gratuit"/><Field label="Nombre de places" value={f.places} onChange={set("places")} placeholder="32"/></div>
        <Field label="Dotations / lots" value={f.dotations} onChange={set("dotations")} placeholder="Coupes, bons cadeaux…"/>
...
const Admin = ({ bars, setBars, associations, setAssociations, tournois, setTournois, setPage, setBarSlug, setAssoSlug, setTournoiSlug }) => {
  ...
  const validerBar=async(prop)=>{ const slug=slugify(prop.nom+"-"+prop.ville); const nb={ slug,nom:prop.nom,ville:prop.ville,cp:prop.cp||"",adresse:prop.adresse||"",tel:prop.tel||"",type:prop.type,cibles:parseInt(prop.cibles)||1,horaires:"",description:"",commentaire:prop.commentaire||"",tournois:prop.tournois==="oui",association:null,source:"user",verifie:false,vues:0,lat:null,lng:null }; const added=await db.addBar(nb); if(added?.[0]) setBars(b=>[...b,added[0]]); await db.updateProposition(prop.id,{statut:"publie"}); setPropositions(p=>p.map(x=>x.id===prop.id?{...x,statut:"publie"}:x)); };
  const validerAsso=async(prop)=>{ const slug=slugify(prop.nom+"-"+prop.ville); const nb={ slug,nom:prop.nom,ville:prop.ville,zone:prop.zone||"",type:prop.type||"electronique",jours:prop.jours||"À confirmer",lieu:prop.lieu||"",tel:prop.tel||"",contact:prop.contact||"",description:prop.description||"",bars:[],source:"user",verifie:true,lat:null,lng:null }; const added=await db.addAssociation(nb); if(added?.[0]) setAssociations(a=>[...a,added[0]]); await db.updateProposition(prop.id,{statut:"publie"}); setPropositions(p=>p.map(x=>x.id===prop.id?{...x,statut:"publie"}:x)); };
  const validerTournoi=async(prop)=>{ const slug=slugify(prop.nom+"-"+prop.ville+"-"+(prop.date||"")); const nb={ slug,nom:prop.nom,ville:prop.ville,date:prop.date||"",bar:prop.bar||"",association:prop.association||"",type:prop.type||"electronique",format:prop.format||"individuel",niveau:prop.niveau||"tous",prix:prop.prix||"",dotations:prop.dotations||"",places:prop.places||"",description:prop.description||"",contact:prop.contact||"",lien:prop.lien||"",source:"user",statut:"publie",lat:null,lng:null }; const added=await db.addTournoi(nb); if(added?.[0]) setTournois(t=>[...t,added[0]]); await db.updateProposition(prop.id,{statut:"publie"}); setPropositions(p=>p.map(x=>x.id===prop.id?{...x,statut:"publie"}:x)); };
  const refuser=async(id)=>{ await db.updateProposition(id,{statut:"refuse"}); setPropositions(p=>p.map(x=>x.id===id?{...x,statut:"refuse"}:x)); };

  const pendingBars=propositions.filter(p=>p.statut==="en_attente"&&p.type_prop!=="association"&&p.type_prop!=="tournoi");
  const pendingAsso=propositions.filter(p=>p.statut==="en_attente"&&p.type_prop==="association");
  const pendingTournoi=propositions.filter(p=>p.statut==="en_attente"&&p.type_prop==="tournoi");
  const allPending=[...pendingBars,...pendingAsso,...pendingTournoi];
        ...
        <div style={{ display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8,marginBottom:10 }}>
          <div>
            <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:4 }}>
              ...
              <Badge color={isAsso?C.purple:isTournoi?C.yellow:C.accent}>{isAsso?"🫂 Asso":isTournoi?"🏅 Tournoi":"🎯 Bar"}</Badge>
            </div>
    ...
    <div style={{ maxWidth:980, margin:"0 auto", padding:"36px 20px" }}>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:12,marginBottom:24 }}>
        ...
        <div style={{ display:"flex",gap:10,flexWrap:"wrap" }}>
          {[[allPending.length,C.yellow,"En attente"],[sigPending.length,C.red,"Signalements"],[bars.length,C.accent,"Bars"]].map(([n,c,l])=>(
      ...
      {loading?<Spinner/>
      :tab==="pending"?(
        allPending.length===0?<div style={{ textAlign:"center",padding:"50px 20px",color:C.muted }}><div style={{ fontSize:42,marginBottom:10 }}>📭</div><p>Aucune proposition en attente.</p></div>
  ...
  <footer style={{ background:"#111",borderTop:`1px solid ${C.border}`,padding:"24px 20px",marginTop:40 }}>
    <div style={{ maxWidth:1100,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12 }}>
      ...
      <div style={{ display:"flex",gap:12,flexWrap:"wrap" }}>
        {[["bars","Bars"],["associations","Assoc."],["tournois","Tournois"],["proposer","Proposer bar"],["proposer-asso","Proposer asso"],["proposer-tournoi","Proposer tournoi"],["apropos","À propos"],["contact","Contact"]].map(([p,l])=><button key={p} onClick={()=>setPage(p)} style={{ background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:12 }}>{l}</button>)}
      </div>
    </div>
  </footer>
);
...
export default function App() {
  ...
  useEffect(()=>{
    Promise.all([db.getBars(),db.getAssociations(),db.getTournois()]).then(([b,a,t])=>{ setBars(b||[]); setAssociations(a||[]); setTournois(t||[]); setLoading(false); }).catch(()=>setLoading(false));
  },[]);
  ...
  if(loading) return <div style={{ height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg,flexDirection:"column",gap:16 }}><span style={{ fontSize:48 }}>🎯</span><div style={{ width:32,height:32,border:`3px solid #2a2a2a`,borderTop:`3px solid #f97316`,borderRadius:"50%",animation:"spin 0.8s linear infinite" }}/><p style={{ color:"#94a3b8",fontSize:14 }}>Chargement…</p><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>;

  return (
    <div style={{ minHeight:"100vh",display:"flex",flexDirection:"column",background:C.bg,color:C.text }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');*{box-sizing:border-box;margin:0;padding:0}html,body,#root{background:#0f0f0f!important;color:#f1f5f9!important;color-scheme:dark}body{font-family:'Inter',sans-serif;-webkit-text-fill-color:#f1f5f9}input,select,textarea,button{font-family:inherit;-webkit-text-fill-color:inherit}::placeholder{color:#94a3b8}::-webkit-scrollbar{width:6px}::-webkit-scrollbar-track{background:#111}::-webkit-scrollbar-thumb{background:#333;border-radius:3px}@keyframes spin{to{transform:rotate(360deg)}}`}</style>
