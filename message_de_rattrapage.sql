...
--  bienvenue vit dans la fonction Edge `auth`, qui n'a jamais été redéployée
--  depuis le 30/07/2026 11h32. Résultat : tous les joueurs inscrits depuis
--  cette date sont arrivés sans aucun accueil (vérifié : 0 sur 72).
--
--  ⚠️ ORDRE À RESPECTER :
--    1. l'appli doit être déployée en v112 AVANT ce SQL, sinon le bouton
--       WhatsApp s'affichera en texte brut « [INVITER] » ;
...
select count(*) as joueurs_a_contacter
 ...
 where j.date_inscription >= 1785403920000
   and j.id <> '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44'
   and not exists (select 1 from public.messages m
                    where m.from_id = '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44' and m.to_id = j.id);


-- ────────────────────────────────────────────────────────────────────────────
-- ÉTAPE 2 — L'AMITIÉ
-- Une seule ligne par joueur, dans le sens Thomas → joueur, comme le fait la
-- fonction `auth`. On saute ceux qui sont déjà amis, dans un sens ou dans l'autre
-- (quelqu'un qui t'aurait retiré volontairement ne sera donc PAS réajouté).
-- ────────────────────────────────────────────────────────────────────────────
...
select '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44', 'Thomas', j.id, j.pseudo, 'accepte',
 ...
 where j.date_inscription >= 1785403920000
   ...
   and not exists (select 1 from public.amis a
                    where (a.joueur_id = '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44' and a.ami_id = j.id)
                       or (a.joueur_id = j.id and a.ami_id = '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44'));
...
select (select count(*) from public.amis
         where joueur_id = '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44')                       as amis_de_thomas,
       (select count(*) from public.messages
         where from_id = '6db6b5b2-bdac-4dc5-9e99-3c1f3a253d44' and lu = false)          as messages_non_lus_envoyes;
